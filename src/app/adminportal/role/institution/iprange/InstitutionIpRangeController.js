    ...
    function (
      ...
      $scope.addConfirm = function () {
        ...
        if ($scope.ipranges.length >= 1) {
          ...
          var currentStartLESSnewStart = IpValidator.CompareIpAddress(
            currentStart,
            newStart
          )
          var currentEndGREATERnewEnd = !IpValidator.CompareIpAddress(
            currentEnd,
            newEnd
          )
          if (currentStartLESSnewStart && currentEndGREATERnewEnd) {
            alert(
              'Error:New Range is already covered by (is within) Current Range'
            )
            return
          }
          ...
          var newStartLESScurrentStart = IpValidator.CompareIpAddress(
            newStart,
            currentStart
          )
          var newEndGREATERcurrentStart = !IpValidator.CompareIpAddress(
            newEnd,
            currentStart
          )
          var newEndLESScurrentEnd = IpValidator.CompareIpAddress(
            newEnd,
            currentEnd
          )
          if (
            newStartLESScurrentStart &&
            newEndGREATERcurrentStart &&
            newEndLESScurrentEnd
          ) {
          ...
          if (
            IpValidator.CompareIpAddress(currentStart, newStart) &&
