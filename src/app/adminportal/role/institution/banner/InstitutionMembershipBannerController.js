  ...
  [
    ...
    function (
      ...
      function saveUploadedImageAndData() {
        ...
        var apiUrl =
          'https://q0b0t0y6i9.execute-api.us-west-2.amazonaws.com/getsignedurl'
        ...
        $http({
          ...
          .success(function (data, status, headers, config) {
            ...
            $http({
              ...
              .error(function (data, status, headers, config) {
                if (data) {
                  bootbox.alert(
                    'There was an error uploading the logo: ' + data['error']
                  )
                } else {
          ...
          .error(function (data, status, headers, config) {
            if (data) {
              bootbox.alert(
                'There was an error uploading the logo. Failed to get signed url: ' +
                  data['error']
              )
            } else {
              bootbox.alert(
      ...
      function saveDataToDB() {
        ...
        if ($scope.isNew) {
          $http({
            url:
              $scope.apiUri +
              '/parties/imageinfo/?credentialId=' +
              $scope.credentialId +
              '&secretKey=' +
          ...
          if (hasChange()) {
            $http({
              ...
              .error(function (data, status, headers, config) {
                if (data) {
                  bootbox.alert(
                    'Failed to update banner info: ' + data['error']
                  )
                } else {
      ...
      function init() {
        ...
        $http({
          ...
          .error(function (data, status, headers, config) {
            ...
            $('#error-msg').show()
            $('#error-msg-text').html(
              'Cannot load banner information. Please refresh the page or try again later.'
            )
          })
      }
      ...
      function initDataModel() {
        var dataModel = Object.assign(
          {},
          InstitutionMembershipBannerModel.imageInfo
        )
