      ...
      function (
        ...
        $scope.licenseAction = function (id) {
          if ($scope.role == 'staff') {
            ...
            //PW-139
            $state.go('role.institution.subscription.renewal', {
              partnerId: id,
            })
          } else {
            $state.go('role.institution.subscription.request', {
              partnerId: id,
            })
          }
        }
        ...
        $scope.listPartners = function (partners) {
          ...
          for (var i = 0; i < partners.length; i++) {
            ...
            partners[i].latestStartDate = $scope.getLatestStartDate(
              partners[i].partnerId
            )
            partners[i].latestEndDate = $scope.getLatestEndDate(
              partners[i].partnerId
            )
            if (partners[i].partnerId in $scope.consActiveSubscriptions) {
              partners[i].consortiumsList =
                $scope.consActiveSubscriptions[partners[i].partnerId]
