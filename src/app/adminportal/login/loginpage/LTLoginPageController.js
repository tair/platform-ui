  ...
  [
    ...
    function (
      ...
      $scope.login = function () {
        ...
        $http({
          url:
            $scope.apiUri + '/credentials/login/?partnerId=' + $scope.partnerId, //phoenix
          data: $scope.formdata,
          method: 'POST',
        })
          .success(function (data, status, headers, config) {
            if ($scope.remember == true) {
              $cookies.org_phoenixbioinformatics_ui_credentialId =
                data['credentialId'] //for user googlestaff it's Credential.partyId and it's 42
              $cookies.org_phoenixbioinformatics_ui_secretKey =
                data['secretKey']
            ...
            if ($cookies.org_phoenixbioinformatics_ui_credentialId != null) {
              ...
              $window.sessionStorage
                .org_phoenixbioinformatics_ui_credentialId != null
            ) {
              $scope.credentialId =
                $window.sessionStorage.org_phoenixbioinformatics_ui_credentialId
            }
              ...
              .success(function (data, status, headers, config) {
                ...
                if ($scope.role == 'staff') {
                  ...
                  $state.go('role.consortium.institution', {
                    consortiumId: $scope.partyInfo.partyId,
                  })
                } else if ($scope.role == 'organization') {
                  $state.go('role.institution.iprange', {
                    institutionId: $scope.partyInfo.partyId,
                  })
                } else {
