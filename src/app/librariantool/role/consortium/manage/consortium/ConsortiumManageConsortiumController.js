...
//        			institution.name = $scope.editInstitution.name;
		...
		$http({
            url: $scope.apiUri+'parties/coinstitution?partyId='+createdInstitution.partyId +'&secretKey='+encodeURIComponent($cookies.secretKey)+'&credentialId='+$cookies.credentialId,
            data:data,
            method: 'PUT',
			}).success(function(data, status, headers, config){
			}).error(function(data, status, headers, config){
