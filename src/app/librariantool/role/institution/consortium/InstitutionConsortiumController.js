angular.module('platform-ui.librariantool.role.institution.consortium').controller('InstitutionConsortiumController', [
  ...
  function ($scope, $http, $cookies, $window, $location, $state, $filter, Title, InstitutionConsortiumModel, IpValidator) {
    $scope.setTitle(InstitutionConsortiumModel.title);
    $scope.ipranges = InstitutionConsortiumModel.ipranges;
    $scope.addGroupShow = 'hidden';
    $scope.adding = false;
    $scope.newRange = InstitutionConsortiumModel.newRange;
    $scope.removeRange = null;
    $scope.editRange = null;
    $scope.searchTerm = null;
    $scope.sortings = InstitutionConsortiumModel.sortings;
    $scope.reverse = $scope.sortings[0].reverse;
    $scope.predicate = $scope.sortings[0].predicate;
    $scope.currentTab = InstitutionConsortiumModel.currentTab;
